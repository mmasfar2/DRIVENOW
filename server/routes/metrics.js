...
router.get('/cashflow', requireAuth, (req, res) => {
  ...
  // Forfeited deposits are their own revenue category — kept out of `actual`
  // (rent collected) so it doesn't distort the expected-vs-actual rent P&L.
  const forfeituresToday = db.prepare(`
    SELECT COALESCE(SUM(forfeited_amount), 0) as total FROM deposits WHERE resolved_at = ?
  `).get(todayStr).total;
  const forfeituresWeek = db.prepare(`
    SELECT COALESCE(SUM(forfeited_amount), 0) as total FROM deposits WHERE resolved_at >= ? AND resolved_at <= ?
  `).get(weekStartStr, todayStr).total;
  const forfeituresMonth = db.prepare(`
    SELECT COALESCE(SUM(forfeited_amount), 0) as total FROM deposits WHERE resolved_at >= ? AND resolved_at <= ?
  `).get(monthStartStr, todayStr).total;
  ...
  const periods = {
    daily: {
      ...
      cardFees: feesToday,
      depositForfeitures: forfeituresToday,
    },
    weekly: {
      ...
      cardFees: feesWeek,
      depositForfeitures: forfeituresWeek,
    },
    monthly: {
      ...
      cardFees: feesMonth,
      depositForfeitures: forfeituresMonth,
    },
  };
  ...
  // Held security deposits are a liability, not revenue — surfaced separately
  // from the periods above so they never get mixed into the P&L figures.
  const depositsHeld = db.prepare(`SELECT COALESCE(SUM(amount), 0) as total FROM deposits WHERE status = 'held'`).get().total;

  res.json({ activeRenters, flaggedRenters, expectedDailyTotal, depositsHeld, periods });
});
